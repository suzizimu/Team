	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td >${student.getIntention()}</td>
				<td ><a href="${student.resume}"  data-lightbox="gallery" ><img width="150px" height="150px" src="${student.getResume()}"></a></td>
				<td >${student.getTel()}</td>
				<td class="td-status"><span class="label label-success radius">${student.getStatus()}</span></td>
				<td class="td-manage"><a style="text-decoration:none" onClick="student_stop(this,${student.getID()})" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a> 
				</td>
			</tr>
	</c:forEach>
...
function admin_bacthdel(){
	...
	var temp=0;
	 ...
	 if(temp==admin_ids.length){
		 ...
		 window.location.href="/Team/Back/studentlist_batchdelete.do?ID="+ID;
	 } 
}
/*用户-全选*/
 function checkedall(){
 ...
 function student_stop(obj,id){
 	var ID=id;
 	 	layer.confirm('确认要停用吗？',function(index){
 		$.post("/Team/Back/student_stop.do",{ID:ID},function(data){
