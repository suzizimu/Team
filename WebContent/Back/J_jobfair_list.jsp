...
<div class="pd-20">
<form action="/Team/Back/jobfair_select.do" method="post">
	...
	<a href="javascript:;" onclick="admin_bacthdel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
	                                                         <a href="javascript:;" onclick="member_add('添加用户','J_jobfair_add.jsp','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加招聘会</a></span> <span class="r">共有数据：<strong>88</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach items="${jobfairlist}" var="fair">
		
			<tr class="text-c">
				<td><input type="checkbox" id="${fair.ID}" value="1" name="admin_ids"></td>
				<td>${fair.ID}</td>
			    <td>${fair.getTitle()}</td>
				<td>${fair.getAddress()}</td> 
				<td >${fair.getFirmNumber()}</td>
				<td >${fair.getPostNumber()}</td>
				<td >${fair.getDate()}</td>
				<td class="td-manage">
				<a title="编辑" href="javascript:;" onclick="admin_edit('管理员编辑','/Team/Back/jobfair_update.do?ID=${fair.getID()}',${fair.getID()},'800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
				<a title="删除" href="javascript:;" onclick="admin_del(this,${fair.getID()})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
	</c:forEach>
			</tr>
		</tbody>
...
function admin_del(obj,id){
	...
	layer.confirm('确认要删除吗？',function(index){
		$.post("/Team/Back/jobfair_delete.do",{ID:ID},function(data){
...
function admin_bacthdel(){
	...
	var temp=0;
	 ...
	 if(temp==admin_ids.length){
		 ...
		 window.location.href="/Team/Back/jobfair_batchdelete.do?ID="+ID;
	 } 
}
/*用户-全选*/
 function checkedall(){
