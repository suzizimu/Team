...
<div class="pd-20">
<form action="/Team/Back/log_select.do" method="post">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				<td><input type="checkbox" id="${log.ID}" value="1" name="admin_ids"></td>				
			    <td>${log.getType()}</td>
				<td>${log.getOperate()}</td> 
				<td >${log.getOperateDate()}</td>
				<td >${log.getAdminName()}</td>
				<td class="td-manage"> 
				<a title="删除" href="javascript:;" onclick="admin_del(this,${log.getID()})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
	</c:forEach>
			</tr>
		</tbody>
...
function admin_del(obj,id){
	...
	layer.confirm('确认要删除吗？',function(index){
		$.post("/Team/Back/log_delete.do",{ID:ID},function(data){
...
function admin_bacthdel(){
	...
	var temp=0;
	 ...
	 if(temp==admin_ids.length){
		 ...
		 window.location.href="/Team/Back/log_batchdelete.do?ID="+ID;
	 } 
}
/*用户-全选*/
 function checkedall(){
