	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td ><a href="${student.resume}"  data-lightbox="gallery" data-title="${student.intention}">${student.getResume()}</a></td>
				<td >${student.getTel()}</td>
				<td class="td-status"><span class="label label-success radius">${student.getStatus()}</span></td>
				<td class="td-manage"><a style="text-decoration:none" onClick="student_start(this,${student.getID()})" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe6e1;</i></a> 
				</td>
			</tr>
	</c:forEach>
			</tr>
...
function admin_bacthdel(){
	...
	var temp=0;
	 ...
	 if(temp==admin_ids.length){
		 ...
		 window.location.href="/Team/Back/student_batchdelbs.do?ID="+ID;
	 } 
}
/*用户-全选*/
 function checkedall(){
 ...
 function student_start(obj,id){
 	...
 	layer.confirm('确认要启用吗？',function(index){
 		$.post("/Team/Back/student_start.do",{ID:ID},function(data){
