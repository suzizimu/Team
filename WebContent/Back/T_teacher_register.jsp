	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			...
			<th width="25"><input type="checkbox" name="" value=""></th>
				...
				<th width="100">年龄</th>
				<th width="100">性别</th>
				<th width="100">教师证件照</th>
				<th width="100">学校工作证</th>
				<th width="100">操作</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${listteacherlist}" var="teacher">
			<tr class="text-c">
				<td><input type="checkbox" id="${teacher.ID}" value="1" name="admin_ids"></td>
				<td>${teacher.ID}</td>
				<td>${teacher.getTeacherCode()}</td>
			    <td><a href="${teacher.getPicture()}"  data-lightbox="gallery" ><img width="150px" height="150px" src="${teacher.getPicture()}"></a></td>
				<td>${teacher.getSex()}</td> 
				<td >${teacher.getAge()}</td>
				<td><a href="${teacher.getQualificationCard()}"  data-lightbox="gallery" ><img width="150px" height="150px" src="${teacher.getQualificationCard()}"></a></td>
				<td><a href="${teacher.getWorkCard()}"  data-lightbox="gallery" ><img width="150px" height="150px" src="${teacher.getWorkCard()}"></a></td>
				<td class="td-manage">
				<a style="text-decoration:none" onClick="student_uncheck(this,${teacher.getID()})" href="javascript:;" title="不通过"><i class="Hui-iconfont">&#xe631;</i></a> 
				<a style="text-decoration:none" onClick="student_checked(this,${teacher.getID()})" href="javascript:;" title="通过"><i class="Hui-iconfont">&#xe6e1;</i></a> 
				<a title="删除" href="javascript:;" onclick="admin_del(this,${teacher.getID()})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
	</c:forEach>
...
function member_stop(obj,id){
	...
	layer.confirm('确认要停用吗？',function(index){
		...
		$(obj).remove();
		$.post("/Team/Back/teacher_stop_start.do",{messege:messege},function(data){
		})
...
function admin_del(obj,id){
	...
	layer.confirm('确认要删除吗？',function(index){
		$.post("/Team/Back/teacherlist_delete.do",{ID:ID},function(data){
	 ...
	 if(temp==admin_ids.length){
		 layer.confirm('确认要删除吗？',function(index){
			...
			layer.confirm('确认要删除吗？',function(index){
				 ...
				 $.post("/Team/Back/teacherlist_batchdelete.do",{ID:ID},function(data){
					 layer.msg('已删除!',{icon:1,time:1000});
			            layer.close(index);
					})				
			});
...
/*用户-全选*/
 ...
 function student_checked(obj,id){
	...
	layer.confirm('确认要通过吗？',function(index){
		$.post("/Team/Back/teacher_checkstatus.do",{messege:messege},function(data){
