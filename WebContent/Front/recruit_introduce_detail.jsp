          ...
          <div class="row">          
            <div class="col-md-10 col-md-offset-0">                      
               <div class="about-author">           
                  <img src="work/天空.jpg" class="img-responsive" alt="" style="width:120px;height:120px;">
                  <h4 class="colored-text">${recruit.getJob() }</h4>
                  <p>${recruit.getFirmName() }||${recruit.getDepartment() }||${recruit.getEducationBgd() }||${recruit.getJobNature() }</p>
                  <p>${recruit.getPay() }||福利：${recruit.getWelfare() }||${recruit.getDate() }</p>                                       
                </div>        
                ...
                <p>
                <c:if test="${student==null }">                
                <a href="../Back/student-login.jsp" class="btn btn-lg btn-theme-bg">收藏该职位</a>
                <a href="../Back/student-login.jsp" class="btn btn-lg btn-theme-bg">投递简历</a>
                </c:if>
                <c:if test="${student!=null&&checkrecruitcollect }">
                <a href="../FrontCollectRecruit.so?id=${recruit.getID()}" class="btn btn-lg btn-theme-bg">收藏该职位</a>
                </c:if>
                <c:if test="${student!=null&&!checkrecruitcollect }">
                <a class="btn btn-lg btn-theme-bg">已收藏</a>
                </c:if>
                <c:if test="${student!=null&&student.getCheckStatus() eq '1'}">
               	<a class="btn btn-lg btn-theme-bg">投递简历（请先进行学生认证）</a>
                </c:if>
                <c:if test="${student!=null&&student.getCheckStatus() eq '0'}">
               	<a class="btn btn-lg btn-theme-bg">投递简历（请先进行学生认证）</a>
                </c:if>
                <c:if test="${student!=null&&student.getCheckStatus() eq '2'&&checkresumesend}">
                <a href="../FrontSendRecruit.so?id=${recruit.getID()}" class="btn btn-lg btn-theme-bg">投递简历</a>
                </c:if>
                <c:if test="${student!=null&&student.getCheckStatus() eq '2'&&!checkresumesend}">
                <a class="btn btn-lg btn-theme-bg">已投递简历</a>
                </c:if>
                </p>
                <br>                                                             
             </div>            
           ...
           <div class="row">
              ...
              <h4>该企业在招职位</h4>
            	<c:forEach items="${recruitlist}" var="rec">
                     <hr>
                   <table width="100%">
                   <tr width="100%"><td align="left" width="50%"><b>${rec.getJob() }</b></td><td width="50%" align="right"><i>${rec.getDate() }</i></td></tr>
                   <tr width="100%"><td align="left" width="50%"><a  class="colored-text" href="../firm_F_informaiton_detail.so?FirmName=${rec.getFirmName() }">${rec.getFirmName() }</a>|${rec.getJobNature() } | ${rec.getEducationBgd() }</td><td width="50%" align="right">${rec.getPay() }</td></tr>
                   </table>
